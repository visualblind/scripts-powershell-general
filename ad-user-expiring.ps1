...
Get-ADUser -SearchBase "OU=Service Accounts,OU=DomainOU,DC=OCTFCU,DC=org" -filter {Enabled -eq $True -and PasswordNeverExpires -eq $False -and PasswordLastSet -gt 0} -Properties * | where {($_.PasswordLastSet).ToShortDateString() -lt $7days} | Select-Object -Property samAccountName,@{Name="ExpiryDate";Expression={[datetime]::FromFileTime($_."msDS-UserPasswordExpiryTimeComputed")}} | Out-Gridview
Get-ADUser -SearchBase "OU=TEST,OU=Users,OU=Admins,DC=OCTFCU,DC=org" -filter {Enabled -eq $True -and PasswordNeverExpires -eq $False -and PasswordLastSet -gt 0} –Properties * | where {($_.PasswordLastSet).ToShortDateString() -eq $7days} | select *




Get-ADUser -SearchBase "OU=Service Accounts,OU=DomainOU,DC=OCTFCU,DC=org" -filter {Enabled -eq $True -and PasswordNeverExpires -eq $False} -Properties "SamAccountName","PasswordLastSet","msDS-UserPasswordExpiryTimeComputed" | 
...
Get-ADUser -SearchBase "DC=OCTFCU,DC=org" -filter {Enabled -eq $True -and PasswordNeverExpires -eq $False} -Properties "SamAccountName","Name","msDS-UserPasswordExpiryTimeComputed" | 
where {
...
$Users = Get-ADUser -SearchBase "OU=Service Accounts,OU=DomainOU,DC=OCTFCU,DC=org" -filter {Enabled -eq $True -and PasswordNeverExpires -eq $False} -Properties msDS-UserPasswordExpiryTimeComputed, PasswordLastSet, CannotChangePassword
...
foreach($u in $Users){
 ...
 Get-ADUser -SearchBase "DC=OCTFCU,DC=org" -filter * -properties Name, PasswordNeverExpires, PasswordExpired, PasswordLastSet | where {$_.Enabled -eq "True"} | where {$_.PasswordNeverExpires -eq $false} | where {$_.passwordexpired -eq $true} | where { 
$diff = New-TimeSpan ([datetime]::FromFileTime($_."msDS-UserPasswordExpiryTimeComputed")) (Get-Date)  
...
Get-ADUser -SearchBase "OU=Service Accounts,OU=DomainOU,DC=OCTFCU,DC=org" -filter {Enabled -eq $True -and PasswordNeverExpires -eq $False} –Properties "DisplayName", "msDS-UserPasswordExpiryTimeComputed" |
 where {
$diff = New-TimeSpan ([datetime]::FromFileTime($_."msDS-UserPasswordExpiryTimeComputed")) (Get-Date)  
$diff.Days -le 99 -and $diff.Days -ge 0
...
Get-ADUser -SearchBase "OU=Tustin,DC=OCTFCU,DC=org" -filter {Enabled -eq $True -and PasswordNeverExpires -eq $False} –Properties "SamAccountName", "DisplayName", "msDS-UserPasswordExpiryTimeComputed" | where {  
$diff = New-TimeSpan ([datetime]::FromFileTime($_."msDS-UserPasswordExpiryTimeComputed")) (Get-Date)
